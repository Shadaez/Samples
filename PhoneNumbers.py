...
text_file = open("phonetext.txt", 'r')

output = []


def phone_number(number, *args):
	global first
	translation =  {'0':['0'],
					'1':['1'],
					'2':['a','b','c'],
	...
	else: #this will run only on the first recursion
		string = ''
		number = number[::-1] #on first run, reverse number

	if len(number) == 1: #this is the only one that should print
		for letter in translation[number[-1:]]:
			output.append(string + letter)
			
	else:
		for letter in translation[number[-1:]]:
			phone_number(number[:-1], string + letter)

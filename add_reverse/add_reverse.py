...
def is_palindrone(number):
	...
	if number[:half_length - (1 - length%2):-1] == number[:half_length]:
		return True
	else:
		return False

def recursive(number, i):
	if is_palindrone(int(number)):
			return number, i
	else:
		return recursive(int(number) + int(str(number)[::-1]), i+1)

text_file = open(sys.argv[1], 'r')

for line in text_file:
	results = recursive(int(line), 0)
